...
int FRAME_BUFFER_insert_text(char *texto, int x, int y){
	if(g_display_element_kop < MAX_ELEMS){
		...
		g_display_elements[g_display_element_kop].what = 1; //1 para text
		g_display_elements[g_display_element_kop].x = x;
		g_display_elements[g_display_element_kop].y = y;
...
/**
 * @brief  Cambiamos el texto de un elemento
 *
 * @return  -
 *
 * Se coge un elemento del tipo what = 1 (texto) indicando su id y se especifica
 * el texto que debe tomar.
 *
*/
void FRAME_BUFFER_change_text(char *texto, int id){

		g_display_elements[id].picture_or_text = texto;
		g_display_elements[id].what = 1; //1 para texto
